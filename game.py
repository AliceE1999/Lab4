...
def remove_punct(text):
    ...
    newText = ""
    for ch in text:
        if 48 <= ord(ch) <= 57 or 65 <= ord(ch) <= 90 or 97 <= ord(ch) <= 122 or ord(ch) == 32:
            newText += ch
    return newText
...
def print_menu(exits):
    ...
    for direction in exits:
        print("Go " + direction.upper() +  " to " + exit_leads_to(exits, direction))
    # COMPLETE THIS PART:
...
def menu(exits):
    ...
    while True:
        print_menu(exits)
        whereyouwannago = input("Type a name of an exit where you want to go: ")
        whereyouwannago = normalise_input(whereyouwannago)
        if is_valid_exit(exits, whereyouwannago):
